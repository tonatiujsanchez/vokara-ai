...
| `PROVIDER_NOT_OFFERED` | 422 | «Ese proveedor todavía no está disponible en Vokara. Elige uno de los que aparecen en la lista.» | Se intentó configurar un proveedor sin verificación empírica registrada, o un endpoint arbitrario (FR-009). El siguiente paso se añadió al implementar T061: la regla 3 de este catálogo no admite excepciones |
...
| `code` | HTTP | Mensaje en español | Cuándo |
|---|---|---|---|
| `EMAIL_APP_PASSWORD_REJECTED` | 400 | «Gmail rechazó la App Password. Si tu cuenta es de Workspace o tiene Protección Avanzada, las App Passwords están deshabilitadas: usa la vía OAuth.» + `details.oauth_docs_url` (dice «de Workspace» y no la marca completa: el guard del art. XI mantiene nombres de proveedor fuera de `domain/`, y la divulgación previa del paso de correo —que sí vive en el adapter— la escribe entera) | Fallo de autenticación IMAP. El aviso ya se dio **antes** de empezar (FR-012); este mensaje lo remata con la salida |
