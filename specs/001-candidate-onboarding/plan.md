...
|---|---|---|
| Renderizador de Markdown propio en `frontend/src/lib/markdown.tsx` (~70 líneas, lista blanca de cuatro construcciones) | La divulgación del art. V viaja como Markdown y hay que pintarla entera (FR-001). El fuente es una **constante del backend**, nunca entrada del usuario, así que el conjunto de construcciones es cerrado y un test lo fija. Sin `dangerouslySetInnerHTML` y sin HTML crudo, en la pantalla donde el usuario decide si confía en Vokara. Lo no soportado se renderiza como texto plano visible: nada se descarta en silencio | `react-markdown`: arrastra `remark`, `unified`, `mdast` y `hast` —decenas de paquetes transitivos— para cuatro construcciones. El criterio de fricción del art. VII pesa sobre el árbol que alguien instala en su máquina, y el intercambio no se sostiene. Es una **dependencia evitada**, no añadida |
