...
- [ ] T015 `infra/docker-compose.yml` con **cuatro servicios y ni uno más**: `api`, `worker`, `postgres` (`pgvector/pgvector:pg16`, tag fijo) y `redis`; `postgres` y `redis` **no se publican en absoluto**; `api` publica en `127.0.0.1:8000:8000` y fija `VOKARA_API_HOST=0.0.0.0` por el proxy de Docker; `api` y `worker` con `env_file: [{ path: ../.env, required: false }]`; **sin ninguna interpolación `${...}`** y **sin `beat`** (research R-20, R-28). Quita el `xfail` del test 1 de T014 y añade `backend/tests/integration/test_compose_env_contract.py`, que falla si aparece un `${` en el Compose o si `api`/`worker` dejan de apuntar a `../.env`
- [ ] T016 `infra/docker-compose.override.yml` con hot reload de desarrollo y el servicio `web` en `127.0.0.1:5173:5173`, dentro del alcance de los tests de T014 y T015
- [ ] T017 `infra/docker/backend.Dockerfile` multi-stage (builder uv → runtime slim), arrancando uvicorn directamente. **Sin migraciones todavía**: el entrypoint que las aplica llega en T024, cuando ya exista una migración que aplicar
