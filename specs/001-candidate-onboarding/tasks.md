...
- [X] T021 Migración `0001_candidate_onboarding` (parte 1) en `backend/app/db/migrations/versions/0001_candidate_onboarding.py`: `CREATE EXTENSION IF NOT EXISTS vector`, los **12 tipos enum** de data-model.md y las **9 tablas** en orden de dependencia (`candidates` → `setup_state`, `provider_configurations`, `candidate_profiles` → `documents` → `parse_jobs` → `profile_entries` → `profile_versions` → `llm_call_logs`)
- [X] T022 Migración `0001` (parte 2), mismo archivo: todos los `CHECK` de data-model.md, índices (incl. `ux_parse_jobs_one_active`, `ix_entries_profile_alive` y el GIN sobre `content`), FK diferida `candidate_profiles.current_version_id` con `use_alter=True`, función y trigger `trg_profile_versions_immutable`, **seed de la fila única de `candidates`**, y `downgrade` inverso exacto
- [X] T023 Test de reversibilidad en `backend/tests/integration/test_migration_reversibility.py`: `upgrade head` → `downgrade base` → `upgrade head` sobre Postgres real, verificando que la extensión, los enums y el trigger quedan como al inicio (DoD de la constitución)
...
- [X] T028 Repositorio base con **`candidate_id` explícito en toda firma** en `backend/app/db/repositories/base.py`, más test en `backend/tests/integration/test_repository_scoping_base.py` que siembra **dos** `candidate_id` y verifica el filtrado (research R-11, FR-049)
...
- [X] T030 `GET /health` en `backend/app/api/v1/health.py` devolviendo `{status, database, migration_revision}` leído de la base, más test de integración en `backend/tests/integration/test_health.py`. **Este es el endpoint que viaja de DB a UI en el checkpoint A**
...
- [ ] T120 [P] **Parcialmente adelantado al bloque A** (2026-08-16): la sección de instalación del `README.md` ya está escrita y verificada contra el checkpoint A —requisitos reales por sistema operativo, versión mínima de Compose, recorrido de `git clone` a `GET /api/v1/health` en un solo comando, ubicación única del `.env` en la raíz, y cómo detener y limpiar—. Se adelantó porque el checkpoint B exige instalación limpia sin editar archivos a mano y esta tarea llegaba al final del bloque C. **Pendiente de esta tarea**: la divulgación del art. V —qué se queda en la máquina, la única excepción, cero telemetría, archivos sin cifrar— (art. V, roadmap §11.1)
