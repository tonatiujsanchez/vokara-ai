...
2. ~~Escribir los ADRs base (001–012) y pivotar la constitución a v2.1.0.~~ ✅ · **Google verificado el 2026-08-11** (`gemini-3.5-flash-lite`: salida estructurada y respeto de `null` en opcionales; `gemini-embedding-001` truncado a 768 dimensiones vía MRL). Es el primer proveedor ofrecible en el wizard, y siendo el default sugerido, desbloquea el paso 1 sin esperar a los demás. **Pendiente: completar la tabla "Estado de verificación" del ADR-011 para los otros cuatro** — probar empíricamente salida estructurada, respeto de `null` en opcionales, embeddings y dimensión en OpenAI, Anthropic, DeepSeek y Kimi. Un proveedor sin verificar no se ofrece en el wizard (§11.2). Dos hallazgos de la verificación de Google aplican al adapter y no solo a la tabla: los parámetros de muestreo están deprecados en Gemini 3.x, y los nombres de modelo no pueden vivir en constantes de código —Google retiró `gemini-2.0-flash` el 1 de junio de 2026— sino en configuración con error accionable.
